...
flags.DEFINE_integer('num_training_steps', int(100000), 'No. of training steps')
...
def learner(model, params):
  ...
  ds_val = dataset.split_and_preprocess(ds_val, noise_field=params['field'],
                                    noise_scale=params['noise'],
                                    noise_gamma=params['gamma'])
  inputs_val = tf.data.make_one_shot_iterator(ds_val).get_next()
  loss_op_val = model.loss(inputs_val)
  ##############################################
  ...
  with tf.train.MonitoredTrainingSession(
    ...
    while not sess.should_stop():
      _, step, loss, loss_val = sess.run([train_op, global_step, loss_op, loss_op_val])
      ########################################################
      #loss_val = sess.run([loss_op_val])
      #########################################################
      if step % 100 == 0:
        logging.info('Step %d: Loss %g', step, loss)
        logging.info('Val_Loss %g', loss_val)
...
def main(argv):
  ...
  if FLAGS.mode == 'train':
    train_loss_list, valid_loss_list = learner(model, params)
    with open('cylinder_loss_train.txt', 'w') as file:
      for item in train_loss_list:
        file.write("%s\n" % item)
    with open('cylinder_loss_valid.txt', 'w') as file:
      for item in valid_loss_list:
        file.write("%s\n" % item)
